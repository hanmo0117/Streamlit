...
st.header('介紹')
st.text('這是一個占卜未來的程式 #測試版 3.0.0\n建議用chrome開啟\n姓和名都是最多2字')

st.header('占卜')

st.write('請先到https://kangxizidian.com/\n查詢姓名的筆畫\n因為Streamlit App不支援WebDriver，所以勞煩了～之後會改善的')
last_name_1 = st.number_input('請輸入你的姓的第1個字的筆劃', min_value = 0, step = 1)
last_name_2 = st.number_input('請輸入你的姓的第2個字的筆劃，沒有則0～', min_value = 0, step = 1)
first_name_1 = st.number_input('請輸入你的名的第1個字的筆劃', min_value = 0, step = 1)
first_name_2 = st.number_input('請輸入你的名的第2個字的筆劃，沒有則0～', min_value = 0, step = 1)
...
if last_name_2 == 0:
    del last_name_2
    Last_Name_Strokes = [last_name_1]
else:
    Last_Name_Strokes = [last_name_1 + last_name_2]
if first_name_2 == 0:
    del first_name_2
    First_Name_Strokes = [first_name_1]
else:
    First_Name_Strokes = [first_name_1, first_name_2]

if len(Last_Name_Strokes) == 1:  # 天格
    Tiange = Last_Name_Strokes[0] + 1
elif len(Last_Name_Strokes) == 2:
    Tiange = Last_Name_Strokes[0] + Last_Name_Strokes[1]
Renge = First_Name_Strokes[-1] + Last_Name_Strokes[0]  # 人格
if len(First_Name_Strokes) == 1:  # 地格
    Dige = First_Name_Strokes[0] + 1
elif len(First_Name_Strokes) == 2:
    Dige = First_Name_Strokes[0] + First_Name_Strokes[1]
Zonge = sum(Last_Name_Strokes) + sum(First_Name_Strokes)  # 總格
if len(Last_Name_Strokes) == 1 or len(First_Name_Strokes) == 1:  # 外格
    Waige = Zonge - Renge + 1
else:
    Waige = Zonge - Renge
...
if st.button('占卜結果'):
    ...
    st.text('如果有任何問題，請聯絡b10203037@ntu.edu.tw\nHan Mo\n'
            '程式碼：https://github.com/hanmo0117/streamlit/blob/develop/%E6%BC%94%E7%AE%97%E6%B3%95%E4%BA%BA%E7%94%9F.Streamlit/main.py\n'
            'APP：https://hanmo0117-streamlit--streamlitmain-develop-20zx8t.streamlit.app/')
